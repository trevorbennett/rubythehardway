...
#will use this .rb to document the file I'm parsing

#here we go: https://github.com/discourse/discourse/blob/master/app/controllers/badges_controller.rb

class BadgesController < ApplicationController #establish class that extends application controller
  skip_before_action :check_xhr, only: [:index, :show] #not sure, not certain what : does in ruby

  def index #define method index
